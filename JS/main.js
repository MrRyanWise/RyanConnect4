...
let playedTurn = RED_TURN;// 1-red, 2-yellow
let hoverColumn = -1;
let animating = false;
...
function onColumnClicked(column){
    //availableRow would take the last position of the 0 
    //[0,0,2,1,2,1].lastIndexOf(0)=1
   ...
   pieces[(availableRow * 7) + column]= playedTurn;
   let cell = board.children[(availableRow * 7) + column];

   let piece = document.createElement("div");
   piece.className = "piece";
   piece.dataset.placed = true;
   piece.dataset.player= playedTurn;
...
function checkGameWinOrDraw(){
    ...
    if(playedTurn === RED_TURN){
        playedTurn = YELLOW_TURN;
    }else{
        playedTurn = RED_TURN;
    }
